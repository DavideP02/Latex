...
\usepackage[fulladjust]{marginnote} %to use marginnote for qed
\newcommand{\QED}{\qed} %new qed definition
...
\newcommand{\teorema}[3][]{
    \stepcounter{teorema}
		...
		#3}
\newcommand{\dimostrazione}[2]{\paragraph{\textit{dim.} \hyperref[thm:#1]{(\Roman{thm#1})}} #2\QED}
...
\newcommand{\proposizione}[3][]{
    \stepcounter{proposizione}
		...
		#3}
\newcommand{\dimostrazioneprop}[2]{\paragraph{\textit{dim.} \hyperref[prp:#1]{(\textit{p.}\roman{prp#1})}} #2\QED}
